...
## Product direction: real estate marketplace + Agency OS
...
matching, inquiries, site visits) on top of the existing **Agency**
operations platform (this was previously a generic "AI for any business"
product - what used to be called a "business" throughout this codebase is
now an **Agency**). See `ARCHITECTURE.md`'s "Real Estate Pivot" section
for the full phase sequence; only Phase 6 (the rename + Buyer auth
foundation) is done so far - everything from Phase 7 onward (Property/
Project inventory, the buyer-facing marketplace frontend, Inquiry→Lead
bridging, site visits/bookings, AI Workforce real-estate personas) is not
started yet.
...
| **WhatsApp/Instagram channel integration**: both channels are adapters on the existing channel-agnostic conversation engine (`process_message_for_agency()`, shared with the website widget - no separate AI logic per channel). Webhook verification, HMAC-SHA256 signature validation (fails closed if unconfigured), inbound normalization, per-agency tenant resolution (`ChannelCredential`, DB-enforced unique per number/account so two agencies can never collide), delivery idempotency (`ChannelWebhookEvent`), conversation reuse without client-side memory, and real (not mocked) outbound sending via Meta's Graph API are all implemented and tested live against real signed HTTP requests. A minimal Settings → Integrations UI lets an agency paste its own connection details and see real connected/disconnected status; Conversations shows each conversation's channel. **Not yet connected to a real Meta account** - no Meta App, Business Verification, or real access token exists for this repository; see `ARCHITECTURE.md`'s Channels section for exactly what that requires | ✅ (repo-side) |
...
5. **Gmail** — ✅ done at the repository level (see Done table above).
   ...
   `ARCHITECTURE.md`.
6. **Agency knowledge base / RAG** — ✅ done (see Done table above,
   "Phase 4 — Agency Knowledge Base + RAG"). `pgvector` is confirmed
...
8. **Real estate pivot, part 1 (Agency rename + Buyer auth foundation)**
   — ✅ done (see Done table above, "Phase 6"). Everything else in the
   real estate pivot sequence is NOT started yet:
   - **Phase 7** — Property/Project inventory (RERA info, price,
     availability, amenities, images, floor plans) + an Agency-side
     Inventory dashboard page, reusing the Knowledge Base's existing
     per-tenant file-storage pattern for uploads.
   - **Phase 8** — the Buyer-facing marketplace frontend becomes the new
     public root at `/` (search/browse/filter, property/project detail
     pages, save/favorite, compare) plus a platform-level, cross-agency
     AI property-matching assistant - architecturally separate from each
     agency's own private, tenant-isolated Knowledge Base, which stays
     untouched.
   - **Phase 9** — an `Inquiry` model bridging a Buyer's interest to an
     Agency's existing `Lead`/CRM; site visits reuse the existing
     `AppointmentService` (a site visit is an appointment with a
     `property_id`, not a new booking engine); a `Booking` model for the
     closed-sale outcome.
   - **Phase 10** — remap the 6 existing generic AI Workforce employees
     onto a real-estate roster (Lead Qualification, Property Matching,
     Sales, Site Visit, Customer Relations, Finance, Documentation,
     Market Intelligence, Real Estate Marketing AI) - new personas/tool
     grants on the existing Manager/Planner/ToolRouter architecture, not
     a new engine.
   - **Phase 11** — extend the existing `WorkflowTriggerType` and
     `NotificationPreference` event vocabularies with real-estate events
     (`inquiry_created`, `site_visit_requested`/`confirmed`,
     `booking_created`) - the Phase 5 engine and Phase 2 notification
     system are reused as-is.
   - **Phase 12** — real estate analytics (inventory velocity,
     inquiry→booking conversion, site-visit show rates) on the existing
     Analytics/dashboard infrastructure.
9. **Human handoff, CRM expansion beyond the real estate pivot's own
   Phase 9, advanced analytics beyond Phase 12, AI agency insights,
   Workforce operations dashboard, employee performance, Voice AI,
   Agency Mode / Autonomous Agency Agent** — NOT STARTED, and explicitly
   excluded from the real estate pivot sequence above.
