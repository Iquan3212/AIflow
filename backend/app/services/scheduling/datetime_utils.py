...
a plain ISO-8601 *local* datetime string, because it is given the agency's
...
agency timezone, and converts to UTC, which is the single canonical form we
...
def parse_local_iso(value: str, tz_name: str | None) -> datetime:
    ...
    agency timezone, and return a timezone-aware UTC datetime.
    ...
    if dt.tzinfo is None:
        # Naive => interpret as local wall-clock in the agency timezone.
...
def to_local(dt_utc: datetime, tz_name: str | None) -> datetime:
    """Render a stored UTC datetime back into the agency timezone."""
