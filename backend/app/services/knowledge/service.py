...
Every method is business_id-scoped; nothing here ever queries or mutates
...
class KnowledgeService:
    ...
    def create(
        self, business_id: str, title: str, filename: str, file_type: str,
    ...
    def get_all(self, business_id: str) -> list[models.KnowledgeDocument]:
        return (
            self.db.query(models.KnowledgeDocument)
            .filter(models.KnowledgeDocument.business_id == business_id)
            .order_by(models.KnowledgeDocument.created_at.desc())
            .all()
        )

    def get(self, document_id: str, business_id: str) -> models.KnowledgeDocument | None:
        return (
            self.db.query(models.KnowledgeDocument)
            .filter(models.KnowledgeDocument.id == document_id, models.KnowledgeDocument.business_id == business_id)
            .first()
        )

    def delete(self, document_id: str, business_id: str) -> bool:
        ...
        document = self.get(document_id, business_id)
        if document is None:
            return False
    ...
    def mark_queued_for_retry(self, document_id: str, business_id: str) -> models.KnowledgeDocument | None:
        document = self.get(document_id, business_id)
        if document is None:
            return None
