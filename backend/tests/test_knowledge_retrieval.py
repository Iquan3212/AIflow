...
throwaway Business, cleaned up after each test. Embeddings are the
...
def business_with_knowledge():
    ...
    doc = models.KnowledgeDocument(
        business_id=biz.id, title="Refund Policy.pdf", filename="Refund Policy.pdf",
    ...
    for i, (text, vector) in enumerate(zip(chunk_texts, vectors)):
        db.add(models.KnowledgeChunk(
            document_id=doc.id, business_id=biz.id, chunk_index=i, content=text, embedding=vector,
        ))
    db.commit()
...
def second_business():
    ...
    cross-tenant isolation - retrieve() for business A must never see
    business B's chunks, regardless of similarity score."""
    db = SessionLocal()
    biz = models.Business(
    ...
    doc = models.KnowledgeDocument(
        business_id=biz.id, title="Secret Internal Memo.pdf", filename="Secret Internal Memo.pdf",
    ...
    text = "Refunds are processed within 5 to 7 business days after approval."  # deliberately near-identical
    vector = _embedder.embed([text])[0]
    db.add(models.KnowledgeChunk(document_id=doc.id, business_id=biz.id, chunk_index=0, content=text, embedding=vector))
    db.commit()
...
class TestRelevantRetrieval:
    def test_a_query_matching_a_chunk_almost_exactly_scores_very_high(self, business_with_knowledge):
        db, biz, doc = business_with_knowledge
        results = retrieve(db, biz.id, "Refunds are processed within 5 to 7 business days after approval.", threshold=0.0)
        assert results
        assert results[0].score > 0.99  # the mock embedder is deterministic - identical text embeds identically
        assert "5 to 7 business days" in results[0].content
        assert results[0].document_name == "Refund Policy.pdf"

    def test_top_k_limits_the_number_of_results(self, business_with_knowledge):
        db, biz, doc = business_with_knowledge
        results = retrieve(db, biz.id, "refund", top_k=1, threshold=0.0)
        assert len(results) <= 1

    def test_empty_query_returns_no_results(self, business_with_knowledge):
        db, biz, doc = business_with_knowledge
        assert retrieve(db, biz.id, "", threshold=0.0) == []
        assert retrieve(db, biz.id, "   ", threshold=0.0) == []

    def test_a_relevance_threshold_of_one_filters_out_everything_but_an_identical_match(self, business_with_knowledge):
        db, biz, doc = business_with_knowledge
...
class TestTenantIsolation:
    def test_retrieval_never_crosses_tenants_even_with_near_identical_content(
        self, business_with_knowledge, second_business,
    ):
        db, biz_a, doc_a = business_with_knowledge
        _, biz_b = second_business

        results_for_a = retrieve(db, biz_a.id, "refund policy business days", threshold=0.0)
        assert all(r.document_name != "Secret Internal Memo.pdf" for r in results_for_a)

        results_for_b = retrieve(db, biz_b.id, "refund policy business days", threshold=0.0)
        assert all(r.document_name != "Refund Policy.pdf" for r in results_for_b)
        # business B's own (near-identical) content is still found for B.
        assert any(r.document_name == "Secret Internal Memo.pdf" for r in results_for_b)

    def test_a_nonexistent_business_id_returns_nothing_not_an_error(self, business_with_knowledge):
        db, biz, doc = business_with_knowledge
        assert retrieve(db, str(uuid.uuid4()), "refund", threshold=0.0) == []


class TestDocumentStatusFiltering:
    def test_chunks_from_a_non_ready_document_are_never_returned(self, business_with_knowledge):
        db, biz, doc = business_with_knowledge
        doc.status = models.KnowledgeDocumentStatus.failed
        db.commit()

        results = retrieve(db, biz.id, "refund policy business days", threshold=0.0)
        assert results == []

    def test_a_deleted_documents_chunks_are_gone_via_cascade(self, business_with_knowledge):
        db, biz, doc = business_with_knowledge
        db.delete(doc)
        db.commit()
        ...
        assert retrieve(db, biz.id, "refund policy business days", threshold=0.0) == []
